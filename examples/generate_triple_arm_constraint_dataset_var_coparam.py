...
def generate(seed):
    ...
    constraint = MultiChainConstraint(['panda_left', 'panda_right', 'panda_top'], 
                                    ...
                                    'panda_top_hand_tcp'],
                                    hand_name=['hand_left', 'hand_right', 'hand_top'], 
                                    hand_joints=[2, 2, 2], 
                                    hand_open = [[0.0325,0.0325,0.0325]], 
                                    hand_closed = [[0.0, 0.0, 0.0]])
    ...
    #     time.sleep(0.3)
    #     c = [0.8, 0.78, 0.66]
    #     p_cg, q_cg = cgc.get_global_grasp(11, c[0], chair_pos, chair_quat)
    ...
    #     p_cg, q_cg = cgc.get_global_grasp(68, c[1], chair_pos, chair_quat)
    ...
    #     p_cg, q_cg = cgc.get_global_grasp(27, c[2], chair_pos, chair_quat)
    ...
    #     print(pc.is_valid(q_2))
    while True:
        # choose constraint condition
        while True:
            c = np.random.uniform([0.2, 0.41, 0.2], [0.8, 0.78, 0.66])
            p_cg, q_cg = cgc.get_global_grasp(11, c[0], chair_pos, chair_quat)
            r, q = constraint.solve_arm_ik('panda_top', q_2[14:21], p_cg, q_cg)
            if r is False:
                continue 
        ...
        # if pc.is_valid(q_2) is False:
        # if pc.display(q_2) is False:
