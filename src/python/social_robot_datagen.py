...
try:
    q_dataset = pickle.load(open('q_dataset_{0}.pkl'.format(z_diff),'rb'))
except:
    q_dataset = []
...
def save_dataset(new_data):
    ...
    pickle.dump(q_dataset, open('q_dataset_{0}.pkl'.format(z_diff),'wb'))
...
pickle.dump(q_dataset_save, open('q_dataset_{0}.pkl'.format(z_diff),'wb'))
